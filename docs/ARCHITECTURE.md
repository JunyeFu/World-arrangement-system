...
OpenCode keeps its full worker prompt in `worker/prompt.md` and attaches that file with `opencode run --file`; this avoids the Windows command-line length limit. For WSL OpenCode, World uses `AI_ORCHESTRATOR_WSL_PROXY` when configured, or safely detects the local Verge/Mihomo proxy on port `7897` before forwarding proxy variables only to that child process.
